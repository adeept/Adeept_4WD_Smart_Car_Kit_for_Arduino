...
void judgement(){
    if (comdata.length() > 0){ 
        ...
        else if(comdata.endsWith(Move_UD_Stop)||comdata.endsWith(Move_LR_Stop)||comdata.endsWith(StopFunction)){//stop
          judge=5;
        }
...
void Avoid_Obstacles(){
    ...
    if (distance > 40){
        ...
        Motor(1,1*dir,0); //Stop the car
        Motor(2,1*dir,0);
        Motor(3,1*dir,0); //Stop the car
        Motor(4,1*dir,0);
        Servo_Angle(1, 45);
        if (distance > 20){
